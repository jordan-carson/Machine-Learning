...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = 1.0/(self.t**2)*self.alpha
            #self.epsilon = self.alpha/((self.t**2)/1.0))
    ...
    def build_state(self):
        ...
        # Done
        # there are some strings that do not have a value, populate with an empty string -> 'None'
                # str -> 'none'
        def strfix(string):
        ...
        ## Done v3 ##
        #############
        state = strfix(waypoint)+"-"+strfix(inputs['light'])+"-"+strfix(inputs['left'])+"-"+strfix(inputs['oncoming'])
    ...
    def get_maxQ(self, state):
        ...
        #state_act_dict = self.Q[state][action]
        maxQ = -1800.0 # initalize MaxQ value
        ...
        for action in self.Q[state]:
            if maxQ < self.Q[state][action]:
                ...
                #Feedback 1: Use the below in replacment of the above
        ...
        if self.learning:
            if state not in self.Q.keys():
                ...
                for action in self.valid_actions:
                    #self.Q[state][action] = 0.0 # set the initial Q-value to 0.0
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            action = random.choice(self.valid_actions) # if not learning, choose random action 
        else:
            #eps_prob = random.random()
            if self.epsilon > 0.01 and self.epsilon > random.random():
                action = random.choice(self.valid_actions) # if learning, choose a random action with 'epsilon' probability
            else:
                vActions = []
                #state_dict = self.Q[state]
                ...
                for act in self.Q[state]:
                    if maxQ == self.Q[state][act]: 
                        vActions.append(act)
                action = random.choice(vActions) # Otherwise, choose an action with the highest Q-value for the current state      
        #Feedback 1: use the below code in replacement of the action = random.choice(vActions)
